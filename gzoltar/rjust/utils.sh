...
_checkout() {
  ...
  # checkout project-bug using defects4j command
  # checkout is seemingly related to "git checkout" command 
  # which is used to switch between branches
...
_get_test_classes_dir() {
  local USAGE="Usage: ${FUNCNAME[0]} <pid> <bid> <checkout_dir> <tool>"
  if [ "$#" != 4 ]; then
    echo "$USAGE" >&2
    return 1
  fi
  ...
  if [ $? -ne 0 ]; then
    ...
    test_classes_dir=".generated_tests"
  fi
...
_collect_list_of_unit_tests() {
  local USAGE="Usage: ${FUNCNAME[0]} <pid> <bid> <checkout_dir> <tool> <output_file>"
  if [ "$#" != 5 ]; then
    echo "$USAGE" >&2
    return 1
  fi
  ...
  if [ "$tool" == "evosuite" ]; then
    [ "$EVOSUITE_RUNTIME_JAR" != "" ] || die "[ERROR] EVOSUITE_RUNTIME_JAR is not set!"
    [ -s "$EVOSUITE_RUNTIME_JAR" ] || die "[ERROR] $EVOSUITE_RUNTIME_JAR does not exist or it is empty!"
  fi
  ...
  test_classes_dir=$(_get_test_classes_dir "$pid" "$bid" "$checkout_dir" "$tool")
  if [ $? -ne 0 ]; then
  ...
  if [ "$tool" == "developer" ]; then
    local relevant_tests_file="$D4J_HOME/framework/projects/$pid/relevant_tests_of_loaded_classes/$bid"
    ...
    local relevant_tests=$(cat "$relevant_tests_file" | sed 's/$/#*/' | sed ':a;N;$!ba;s/\n/:/g')
    echo "[DEBUG] relevant_tests: $relevant_tests" >&2
  else
    local relevant_tests="*" # All
  fi
  ...
  java -cp "$test_classpath:$test_classes_dir:$D4J_HOME/framework/projects/lib/junit-4.11.jar:$EVOSUITE_RUNTIME_JAR:$GZOLTAR_CLI_JAR" \
    com.gzoltar.cli.Main listTestMethods \
      "$test_classes_dir" \
      --outputFile "$output_file" \
      --includes "$relevant_tests" || die "GZoltar listTestMethods command has failed!"
...
_collect_list_of_likely_faulty_classes() {
  ...
  # information about classes in the target <pid> class is stored in the $bid.src
  ...
  # "normal" classes
  # SED command
  # sed 's/$/:/' : replace "$" with ":" at the end of each line ($ represents the end of the line in UNIX)
  ## sed 's/<search-pattern>/<replace-pattern>/<options>'
  # sed ':a;N;$!ba;s/\n//g'
  ## ":a;" : start a label named "a"
  ## "N;" 
  ## "$!ba;" : 
  ## "s/\n//g" : replace all new lines with empty string (g stands for global)
  ...
  echo "[DEBUG] likely faulty classes: $normal_classes$inner_classes" >&2
...
#
_run_gzoltar() {
  local USAGE="Usage: ${FUNCNAME[0]} <pid> <bid> <data_dir> <tool> <tests archive>"
  if [ "$#" != 4 ] && [ "$#" != 5 ]; then
    echo "$USAGE" >&2
    return 1
  fi

  # environment settings for running GZoltar
  [ "$D4J_HOME" != "" ] || die "[ERROR] D4J_HOME is not set!"
  [ -d "$D4J_HOME" ] || die "[ERROR] $D4J_HOME does not exist!"
  ...
  # local variables which stores the input parameters
  local pid="$1"
  local bid="$2"
  local data_dir="$3"
  local tool="$4"
  local tests_archive="$5"

  # if tests are generated by evosuite, then set the classpath
  if [ "$tool" == "evosuite" ]; then
    [ "$EVOSUITE_RUNTIME_JAR" != "" ] || die "[ERROR] EVOSUITE_RUNTIME_JAR is not set!"
    [ -s "$EVOSUITE_RUNTIME_JAR" ] || die "[ERROR] $EVOSUITE_RUNTIME_JAR does not exist or it is empty!"
  fi

  # checkout the project by calling _checkout function
  # _checkout function is defined at line 114
  local tmp_dir=""
  ...
  # create classes_to_debug.txt file
  ...
  # call _collect_list_of_likely_faulty_classes function
  ...
  if [ "$tool" == "developer" ]; then
    ...
    [ -s "$tests_archive" ] || die "[ERROR] $tests_archive does not exist or it is empty!"

    # Unpack generated tests
    local generated_tests_dir="$tmp_dir/.generated_tests"
    mkdir -p "$generated_tests_dir"
    tar -xf "$tests_archive" -C "$generated_tests_dir" >&2 || die "[ERROR] Failed to extract $tests_archive to $generated_tests_dir!"

    # Compile generated tests
    find "$generated_tests_dir" -name "*.java" | xargs javac -cp \
      "$src_classes_dir:$generated_tests_dir:$test_classpath:$D4J_HOME/framework/projects/lib/junit-4.11.jar:$EVOSUITE_RUNTIME_JAR" || die "[ERROR] Failed to compile the generated test suites!"
    ...
    local instr_src_classes_dir="$tmp_dir/.instrumented_classes"
    mkdir -p "$instr_src_classes_dir"
    (cd "$tmp_dir" > /dev/null 2>&1 && \
      java -cp "$src_classes_dir:$test_classpath:$GZOLTAR_AGENT_JAR:$GZOLTAR_CLI_JAR" \
        com.gzoltar.cli.Main instrument \
        --outputDirectory "$instr_src_classes_dir" \
        "$src_classes_dir") || die "[ERROR] Offline instrumentation of all classes in $src_classes_dir has failed!"
    ...
    # Run GZoltar
    echo "[INFO] Start: $(date)" >&2
    local runTestMethodsOptions="--offline --collectCoverage"
    if [ "$tool" == "evosuite" ]; then
      runTestMethodsOptions="$runTestMethodsOptions --initTestClass"
    fi
    (cd "$tmp_dir" > /dev/null 2>&1 && \
      java -XX:MaxPermSize=4096M \
        -cp "$instr_src_classes_dir:$generated_tests_dir:$test_classpath:$D4J_HOME/framework/projects/lib/junit-4.11.jar:$EVOSUITE_RUNTIME_JAR:$GZOLTAR_AGENT_JAR:$GZOLTAR_CLI_JAR" \
        -Dgzoltar-agent.destfile="$ser_file" \
        -Dgzoltar-agent.output="file" \
        -Dgzoltar-agent.buildlocation="$instr_src_classes_dir" \
        -Dgzoltar-agent.includes="$classes_to_debug" \
        -Dgzoltar-agent.excludes="" \
        -Dgzoltar-agent.inclnolocationclasses="false" \
        -Dgzoltar-agent.exclClassLoader="sun.reflect.DelegatingClassLoader" \
        com.gzoltar.cli.Main runTestMethods \
          --testMethods "$unit_tests_file" $runTestMethodsOptions)
    if [ $? -ne 0 ]; then
      echo "[ERROR] GZoltar runTestMethods command has failed for $pid-${bid}b version!" >&2
      rm -rf "$tmp_dir"
      return 1
